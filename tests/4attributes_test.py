...
def test_node_attributes(datadir_mgr):
    ...
    with datadir_mgr.in_tmp_dir(
        ...
        output = sh.bionorm(["node-attributes", "--recurse", "--pathonly", ORGANISM_DIR])
        lines_out = output.count("\n")
        assert lines_out == len(test_file_paths)
        assert not sh.bionorm(["node-attributes", "--recurse", "--pathonly", "--invalid", ORGANISM_DIR]).count(
            "\n"
        )  # no invalid filename
        assert not sh.bionorm(["node-attributes", "--recurse", "--pathonly", "--unrecognized", ORGANISM_DIR]).count(
            "\n"
        )  # no unrecognized filenames
        UNREC_PATH.touch()
        assert 1 == sh.bionorm(["node-attributes", "--recurse", "--pathonly", "--unrecognized", ORGANISM_DIR]).count(
            "\n"
        )  # now 1 unrecognized filename
        UNREC_PATH.unlink()
        for badname, reason in INVALID_LIST:
            badpath = ANN_PATH / badname
            badpath.touch()
            print(f"testing {reason}")
            jsondict = sh.bionorm(["node-attributes", "--recurse", "--json", "--invalid", ORGANISM_DIR])
